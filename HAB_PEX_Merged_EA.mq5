...
//+------------------------------------------------------------------+
#property strict
#property version "2.400"
#property description "HAB PEX Merged EA v2.4\n11-Level Pivot Ladder (S5..P..R5)\nAnchor + Basket Terminator + Manual + Auto Trade Management\nOptimized: KillSwitch, TrendFlip, FreezeCheck, MagicFilter"
...
input bool            Inp_BasketUsePct         = false;    // استفاده از درصد اکوئیتی به جای دلار
input double          Inp_BasketTP_Pct         = 0.5;      // هدف سود سبد (٪ اکوئیتی)
input double          Inp_BasketSL_Pct         = -1.5;     // حد ضرر سبد (٪ اکوئیتی — منفی)
input bool            Inp_BasketMagicOnly      = false;    // فقط معاملات EA در محاسبه سبد
...
input bool            Inp_CloseAllOnDailyLoss  = false;    // هنگام رسیدن به سقف روزانه، همه معاملات بسته شود
input bool            Inp_CloseAllOnWeeklyLoss = false;    // هنگام رسیدن به سقف هفتگی، همه معاملات بسته شود
//--- (۱.۵) Equity Kill-Switch — محافظت کلی حساب
input bool            Inp_EnableKillSwitch     = true;     // فعال‌سازی Kill-Switch اکوئیتی
input double          Inp_KillSwitch_DDPct     = 15.0;     // درصد افت از اوج اکوئیتی برای توقف کامل
...
// --- Equity Kill-Switch ---
double   g_ksPeakEquity = 0.0;          // اوج اکوئیتی
bool     g_ksTriggered  = false;        // آیا Kill-Switch فعال شده
...
void Append(Level &arr[], const Level &x)
  {
   int n = ArraySize(arr);
   ArrayResize(arr, n + 1, 64);  // reserve 64 extra slots to reduce reallocations
   arr[n] = x;
  }
      ...
      if(ddPct >= Inp_WeeklyLossLimitPct && !g_weeklyHalt)
        {
         ...
         if(Inp_CloseAllOnWeeklyLoss) CloseAllSymbol(true);
        }
     }
  }
...
// --- Equity Kill-Switch: drawdown from peak equity ---
void UpdateKillSwitch()
  {
   if(!Inp_EnableKillSwitch) { g_ksTriggered = false; return; }

   double eq = AccountInfoDouble(ACCOUNT_EQUITY);
   if(eq <= 0) return;

   if(eq > g_ksPeakEquity) g_ksPeakEquity = eq;

   if(g_ksPeakEquity > 0 && !g_ksTriggered)
     {
      double ddPct = (g_ksPeakEquity - eq) / g_ksPeakEquity * 100.0;
      if(ddPct >= Inp_KillSwitch_DDPct)
        {
         g_ksTriggered = true;
         Log(StringFormat("EQUITY KILL-SWITCH: DD=%.2f%% >= %.2f%% from peak $%.2f => HALT ALL",
                          ddPct, Inp_KillSwitch_DDPct, g_ksPeakEquity));
         CloseAllSymbol(true);
        }
     }
  }

bool KillSwitchBlocksEntry()
  {
   if(!Inp_EnableKillSwitch) return false;
   return g_ksTriggered;
  }
...
bool RegimeBlocksEntry()
  {
   ...
   if(g_atrEA == INVALID_HANDLE || g_atrNowEA <= 0) return false;
   ...
   ArrayResize(b, p);
   ArraySetAsSeries(b, true);
   int got = CopyBuffer(g_atrEA, 0, 1, p, b);
...
void AutoTradeEngine()
  {
   ...
   // Direction comes from anchor — but check for trend flip
   int dir = g_anchorDir;
   if(dir == 0) return;

   // Trend-flip safety: if MA trend reversed vs anchor direction, skip new ladder entries
   int curTrend = TrendDir();
   if(curTrend != 0 && curTrend != dir)
     { Log("AutoTrade: trend flipped vs anchor => skip new ladder entries"); return; }
   ...
   for(int idx = startIdx; (step == -1 ? idx >= endIdx : idx <= endIdx); idx += step)
     {
      ...
      double distPts = MathAbs(lvl - cur) / _Point;
      int stopLevel = (int)SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL);
      int freezeLevel = (int)SymbolInfoInteger(_Symbol, SYMBOL_TRADE_FREEZE_LEVEL);
      int minPtsReq = MathMax(Inp_MinDist_Points, MathMax(stopLevel, freezeLevel));
      if(distPts < minPtsReq) continue;
...
//=====================================================================
// Calculate total PnL of open positions for this symbol (optionally filter by magic)
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ...
      if(tk == 0 || !PositionSelectByTicket(tk)) continue;
      if(PositionGetString(POSITION_SYMBOL) != _Symbol) continue;
      if(Inp_BasketMagicOnly && (ulong)PositionGetInteger(POSITION_MAGIC) != Inp_MagicNumber) continue;
...
// Close all positions for this symbol (basket close) — respects magic filter
...
void BasketTerminator()
  {
   ...
   // Compute effective thresholds (USD or percentage-based)
   double effTP = Inp_BasketTP_USD;
   double effSL = Inp_BasketSL_USD;
   double effTrailStart = Inp_BasketTrailStart_USD;
   double effTrail = Inp_BasketTrail_USD;
   if(Inp_BasketUsePct)
     {
      double eq = AccountInfoDouble(ACCOUNT_EQUITY);
      if(eq > 0)
        {
         effTP = eq * Inp_BasketTP_Pct / 100.0;
         effSL = eq * Inp_BasketSL_Pct / 100.0;  // negative
         effTrailStart = eq * MathAbs(Inp_BasketTP_Pct) * 0.5 / 100.0; // 50% of TP pct
         effTrail = effTrailStart * 0.6;  // 60% of trail start
        }
     }

   // --- Hard Stop Loss (basket) ---
   if(pnl <= effSL && Inp_BasketCloseOnHit)
     {
      CloseAllBasket(StringFormat("BASKET SL HIT: PnL=%.2f <= %.2f", pnl, effSL));
      return;
     }
   ...
   if(!g_basketTrailing && pnl >= effTrailStart)
     {
      g_basketTrailing = true;
      Log(StringFormat("BASKET TRAIL ACTIVATED: PnL=%.2f >= %.2f", pnl, effTrailStart));
     }

   // --- Take Profit (basket) ---
   if(pnl >= effTP && Inp_BasketCloseOnHit)
     {
      CloseAllBasket(StringFormat("BASKET TP HIT: PnL=%.2f >= %.2f", pnl, effTP));
      return;
     }
...
void Process()
  {
   ...
   UpdateKillSwitch();
   if(KillSwitchBlocksEntry())
     {
      ManageManualTrades();
      ManageEAPositions();
      BasketTerminator();
      UpdateStatusComment();
      return;  // halt all new entries
     }
...
// --- نمایش وضعیت محافظ‌ها/فیلترها روی چارت ---
void UpdateStatusComment()
  {
   string s = "HAB PEX Merged v2.4 (Anchor + Basket + KillSwitch)\n";
   s += StringFormat("AutoTrade: %s | Positions: %d/%d\n",
                     (Inp_EnableAutoTrade ? "ON" : "OFF"),
                     CountAllActiveSymbol(), Inp_MaxPositions);

   // Anchor status
   if(g_anchorActive)
      s += StringFormat("ANCHOR: %s #%I64u @ %.2f (lot=%.2f)\n",
                        (g_anchorDir == 1 ? "BUY" : "SELL"),
                        g_anchorTicket, g_anchorEntry, g_anchorLot);
   else
      s += "ANCHOR: --- (waiting for 0.01 trade)\n";

   // Basket status
   if(Inp_EnableBasket)
     {
      double bPnl = CalcBasketPnL();
      s += StringFormat("BASKET: PnL=$%.2f | TP=$%.0f | SL=$%.0f | HWM=$%.2f %s\n",
                        bPnl, Inp_BasketTP_USD, Inp_BasketSL_USD,
                        g_basketHighWaterMark,
                        (g_basketTrailing ? "[TRAILING]" : ""));
     }

   if(Inp_EnableRiskGuard)
     {
      double eq = AccountInfoDouble(ACCOUNT_EQUITY);
      ...
      s += StringFormat("RiskGuard: dayDD %.2f%%/%.1f%% %s | weekDD %.2f%%/%.1f%% %s\n",
                        dPct, Inp_DailyLossLimitPct,  (g_dailyHalt  ? "[HALT]" : ""),
                        wPct, Inp_WeeklyLossLimitPct, (g_weeklyHalt ? "[HALT]" : ""));
     }

   // Kill-Switch status
   if(Inp_EnableKillSwitch)
     {
      double eqNow = AccountInfoDouble(ACCOUNT_EQUITY);
      double ksDDPct = (g_ksPeakEquity > 0) ? (g_ksPeakEquity - eqNow) / g_ksPeakEquity * 100.0 : 0.0;
      s += StringFormat("KillSwitch: DD %.2f%%/%.1f%% %s\n",
                        ksDDPct, Inp_KillSwitch_DDPct, (g_ksTriggered ? "[HALT]" : ""));
     }

   string gates = "";
   if(g_blkCB)     gates += "CB ";
   if(g_blkNews)   gates += "News ";
   if(g_blkRegime) gates += "Regime ";
   if(g_ksTriggered) gates += "KillSwitch ";
   bool dPaused = (GlobalVariableCheck("HAB_PEX_PAUSED") && GlobalVariableGet("HAB_PEX_PAUSED") > 0.5);
   if(dPaused)     gates += "PAUSED ";
   if(StringLen(gates) > 0) s += "Blocked: " + gates + "\n";
...
int OnInit()
  {
   ...
   // g_atrRegime removed — reuse g_atrEA (same TF/period)
   ...
   // Initialize Kill-Switch peak equity
   g_ksPeakEquity = AccountInfoDouble(ACCOUNT_EQUITY);
   g_ksTriggered = false;

   Log("EA INIT OK — HAB PEX Merged v2.4 (Anchor + Basket + KillSwitch + TrendFlip + FreezeCheck)");
...
void OnDeinit(const int reason)
  {
   ...
   // g_atrRegime removed — shared with g_atrEA
...
void OnTick()
  {
   // Trade management on every tick for faster response
   Process();
  }
...
void OnChartEvent(const int id,
                  ...
                  const string &sparam)
  {
   // placeholder for future UI buttons
  }
//+------------------------------------------------------------------+
